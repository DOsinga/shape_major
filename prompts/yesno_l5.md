# Major System yes/no filter — length 5 single-candidate buckets

Each line of the chunk file you're given is a triple: `<pattern>\t<word>\t<frequency>`. The word is the *only* candidate in its length-5 bucket. Your job is to decide for each one: is the word a usable Major System mnemonic?
...
- A well-known abbreviation/initialism (`USDA`, `LBJ`, `IMDB`-style — but at length 5, these are rare)
...
The orchestrator gives you a chunk file path like `/tmp/l5_yesno/NNN.txt`. Each line is `<pattern>\t<word>\t<frequency>`.

1. Read your chunk file.
2. For each line, decide KEEP or DROP using the rules above.
3. Write the list of patterns to DROP (one 5-digit pattern per line, nothing else) to `/tmp/l5_drops/NNN.txt` — same `NNN` as your input chunk.

If the input is `/tmp/l5_yesno/007.txt`, output to `/tmp/l5_drops/007.txt`.
