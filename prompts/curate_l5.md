# Major System mnemonic word curation — length 5 multi-candidate buckets

Curate length-5 word lists for a Major System mnemonic web app. Mapping (vowels ignored):
...
The orchestrator gives you a chunk file like `/tmp/l5_curate/NNN.txt` containing ~170 five-digit pattern names, one per line.

For each pattern XXXXX in your chunk file:

1. Read `/Users/douwe/Dropbox/Douwe/Proj/memory_app/prioritized/5/XXXXX.txt`. Format: `word<TAB>frequency`. **The file already has 2+ candidates pre-filtered to freq ≥ 40K.**
